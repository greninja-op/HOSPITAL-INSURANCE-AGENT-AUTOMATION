 ...
 *
 * `parseInbound` is a TOTAL function: it never drops a message and never throws.
 * Anything it cannot interpret becomes `kind: "unsupported"` with an empty body, so
 * the dedupe + audit trail still records that something arrived (Requirements 32.1, 32.2).
 */
